...
public class ProduksiPanel extends JPanel {
    private static final String TITLE = "Produksi";
    private static final String BTN_EXPORT = "Export PDF";
    private static final String BTN_ADD = "+ Tambah Produksi";
    private static final int SCROLL_SPEED = 16;
    private static final int TIMER_DELAY_MS = 60_000;

    private final ProduksiDAO produksiDAO = new ProduksiDAO();
    private JLabel lblTotalProduksi;
    private JLabel lblHasilTotal;
    private JLabel lblOperatorAktif;
    private ActivityTable tableProduksi;

    public ProduksiPanel(String userName, String userRole) {
    ...
    private void initializeDynamicComponents() {
        lblTotalProduksi = createAnimatedLabel();
        lblHasilTotal = createAnimatedLabel();
        lblOperatorAktif = createAnimatedLabel();
    }
    ...
    private JPanel buildMainContent() {
        ...
        container.add(buildProduksiTable());
    ...
    private JPanel buildHeaderActionPanel() {
        ...
        buttonPanel.add(createHeaderButton(BTN_ADD, true, this::handleAddProduksi));
        return buttonPanel;
    }
    ...
    private JPanel buildTopCards() {
        ...
        topCards.add(createStatCard("TOTAL PRODUKSI", lblTotalProduksi, "Jumlah proses", Theme.BLUE_ACCENT));
        topCards.add(createStatCard("HASIL TOTAL", lblHasilTotal, "Tahu dihasilkan", Theme.GREEN));
        topCards.add(createStatCard("OPERATOR AKTIF", lblOperatorAktif, "Pengguna terlibat", Theme.WARNING));
        return topCards;
    }

    private ActivityTable buildProduksiTable() {
        String[] headers = {"ID", "Batch", "Tanggal", "Produk", "Hasil", "Status", "Aksi"};
        tableProduksi = new ActivityTable("Daftar Produksi", headers, -1, new ActivityTable.DataProvider() {
            @Override
            public int getTotalRowCount(String keyword) {
                return produksiDAO.getTableTotalRows(keyword);
            }

            @Override
            public java.util.List<String[]> getPageData(int limit, int offset, String keyword) {
                return produksiDAO.getTablePageData(limit, offset, keyword);
            }
        });
        tableProduksi.setTableActionListener((id, name) -> handleViewDetail(id, name));
        return tableProduksi;
    }

    private void handleViewDetail(String id, String name) {
        JOptionPane.showMessageDialog(this, "Detail produksi id " + id + " akan dibuat segera.", "Info", JOptionPane.INFORMATION_MESSAGE);
    }

    private void handleExportPDF() {
        JOptionPane.showMessageDialog(this, "Fitur export PDF produksi sedang dalam pengembangan.", "Info", JOptionPane.INFORMATION_MESSAGE);
    }
    ...
    private void refreshData() {
        ...
        if (tableProduksi != null) {
            tableProduksi.updateTableModel();
        }
    }
    ...
    private void fetchTopCardsData() {
        new SwingWorker<Map<String, String>, Void>() {
            ...
            protected Map<String, String> doInBackground() {
                return produksiDAO.getTopCardsData();
            }
            ...
            protected void done() {
                try {
                    ...
                    lblTotalProduksi.setText(data.getOrDefault("total_produksi", "0"));
                    lblHasilTotal.setText(data.getOrDefault("hasil_total", "0"));
                    lblOperatorAktif.setText(data.getOrDefault("operator_aktif", "0"));
                } catch (Exception e) {
                    e.printStackTrace();
                }
            }
