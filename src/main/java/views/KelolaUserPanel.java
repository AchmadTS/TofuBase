...
public class KelolaUserPanel extends JPanel {
    private static final String TITLE = "Kelola User";
    private static final String BTN_EXPORT = "Export PDF";
    private static final String BTN_ADD = "+ Tambah User";
    private static final int SCROLL_SPEED = 16;
    private static final int TIMER_DELAY_MS = 60_000;

    private final UserManagementDAO userDao = new UserManagementDAO();
    private JLabel lblTotalUsers;
    private JLabel lblAdminUsers;
    private JLabel lblStaffUsers;
    private ActivityTable tableUsers;

    public KelolaUserPanel(String userName, String userRole) {
    ...
    private void initializeDynamicComponents() {
        lblTotalUsers = createAnimatedLabel();
        lblAdminUsers = createAnimatedLabel();
        lblStaffUsers = createAnimatedLabel();
    }
    ...
    private JPanel buildHeaderActionPanel() {
        ...
        buttonPanel.add(createHeaderButton(BTN_ADD, true, this::handleAddUser));
        return buttonPanel;
    }
    ...
    private JPanel buildTopCards() {
        ...
        topCards.add(createStatCard("TOTAL USER", lblTotalUsers, "Akun terdaftar", Theme.BLUE_ACCENT));
        topCards.add(createStatCard("ADMIN", lblAdminUsers, "Akun admin", Theme.GREEN));
        topCards.add(createStatCard("STAFF", lblStaffUsers, "Akun staff", Theme.WARNING));
        return topCards;
    }

    private ActivityTable buildUserTable() {
        String[] headers = {"ID", "Username", "Nama", "Email", "Telepon", "Peran", "Aksi"};
        tableUsers = new ActivityTable("Daftar User", headers, -1, new ActivityTable.DataProvider() {
            @Override
            public int getTotalRowCount(String keyword) {
                return userDao.getTableTotalRows(keyword);
            }

            @Override
            public java.util.List<String[]> getPageData(int limit, int offset, String keyword) {
                return userDao.getTablePageData(limit, offset, keyword);
            }
        });
        tableUsers.setTableActionListener((id, name) -> handleViewDetail(id, name));
        return tableUsers;
    }

    private void handleViewDetail(String id, String name) {
        JOptionPane.showMessageDialog(this, "Detail user id " + id + " belum tersedia.", "Info", JOptionPane.INFORMATION_MESSAGE);
    }

    private void handleExportPDF() {
        JOptionPane.showMessageDialog(this, "Fitur export PDF user sedang dalam pengembangan.", "Info", JOptionPane.INFORMATION_MESSAGE);
    }

    private void handleAddUser() {
        JOptionPane.showMessageDialog(this, "Fitur tambah user akan segera tersedia.", "Info", JOptionPane.INFORMATION_MESSAGE);
    }

    private void refreshData() {
        fetchTopCardsData();
        if (tableUsers != null) {
            tableUsers.updateTableModel();
        }
    }
    ...
    private void fetchTopCardsData() {
        new SwingWorker<Map<String, String>, Void>() {
            ...
            protected Map<String, String> doInBackground() {
                return userDao.getTopCardsData();
            }
            ...
            protected void done() {
                try {
                    ...
                    lblTotalUsers.setText(data.getOrDefault("total_users", "0"));
                    lblAdminUsers.setText(data.getOrDefault("admin_users", "0"));
                    lblStaffUsers.setText(data.getOrDefault("staff_users", "0"));
                } catch (Exception e) {
                    e.printStackTrace();
                }
            }
