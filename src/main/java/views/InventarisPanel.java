...
public class InventarisPanel extends JPanel {
    private static final String TITLE = "Inventaris";
    private static final String BTN_EXPORT = "Export PDF";
    private static final String BTN_ADD = "+ Tambah Inventaris";
    private static final int SCROLL_SPEED = 16;
    private static final int TIMER_DELAY_MS = 60_000;

    private final InventarisDAO inventarisDAO = new InventarisDAO();
    private JLabel lblTotalCek;
    private JLabel lblTerakhirCek;
    private JLabel lblAktivitasBulanan;
    private ActivityTable tableInventaris;

    public InventarisPanel(String userName, String userRole) {
    ...
    private void initializeDynamicComponents() {
        lblTotalCek = createAnimatedLabel();
        lblTerakhirCek = createAnimatedLabel();
        lblAktivitasBulanan = createAnimatedLabel();
    }
    ...
    private JPanel buildMainContent() {
        ...
        container.add(buildInventarisTable());
    ...
    private JPanel buildHeaderActionPanel() {
        ...
        buttonPanel.add(createHeaderButton(BTN_ADD, true, this::handleAddInventaris));
        return buttonPanel;
    }
    ...
    private JPanel buildTopCards() {
        ...
        topCards.add(createStatCard("TOTAL CEK", lblTotalCek, "Sesi pemeriksaan", Theme.BLUE_ACCENT));
        topCards.add(createStatCard("TERAKHIR CEK", lblTerakhirCek, "Tanggal terakhir", Theme.GREEN));
        topCards.add(createStatCard("AKTIVITAS BULANAN", lblAktivitasBulanan, "Pemeriksaan bulan ini", Theme.WARNING));
        return topCards;
    }

    private ActivityTable buildInventarisTable() {
        String[] headers = {"ID", "Tanggal Cek", "Keterangan", "Aksi"};
        tableInventaris = new ActivityTable("Daftar Inventaris", headers, -1, new ActivityTable.DataProvider() {
            @Override
            public int getTotalRowCount(String keyword) {
                return inventarisDAO.getTableTotalRows(keyword);
            }

            @Override
            public java.util.List<String[]> getPageData(int limit, int offset, String keyword) {
                return inventarisDAO.getTablePageData(limit, offset, keyword);
            }
        });
        tableInventaris.setTableActionListener((id, name) -> handleViewDetail(id, name));
        return tableInventaris;
    }

    private void handleViewDetail(String id, String name) {
        JOptionPane.showMessageDialog(this, "Detail inventaris id " + id + " belum tersedia.", "Info", JOptionPane.INFORMATION_MESSAGE);
    }

    private void handleExportPDF() {
        JOptionPane.showMessageDialog(this, "Fitur export PDF inventaris sedang dalam pengembangan.", "Info", JOptionPane.INFORMATION_MESSAGE);
    }
    ...
    private void fetchTopCardsData() {
        new SwingWorker<Map<String, String>, Void>() {
            ...
            protected Map<String, String> doInBackground() {
                return inventarisDAO.getTopCardsData();
            }
            ...
            protected void done() {
                try {
                    ...
                    lblTotalCek.setText(data.getOrDefault("total_cek", "0"));
                    lblTerakhirCek.setText(data.getOrDefault("terakhir_cek", "-"));
                    lblAktivitasBulanan.setText(data.getOrDefault("aktivitas_bulanan", "0"));
                } catch (Exception e) {
                    e.printStackTrace();
                }
            }
